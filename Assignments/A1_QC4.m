...
for k = 1:length(nu_values)
    ...
    initial_conditions = [R0, theta0, V_theta0, V_R0, alpha_T0];

    % Set event function to stop integration when R = 0 (interception)
    options = odeset('Events', @event_function);

    % Solve the differential equations using ode45
    [t, state, te, ye, ie] = ode45(@(t, y) PP(t, y, VP, VT), tspan, initial_conditions, options);
    ...
    % Plot results
    t = plot_results(t, R, theta, V_theta, V_R, aP, te);
...
function t=plot_results(t, R, theta, V_theta, V_R, aP, te)
    % Check if interception occurred
    if ~isempty(te)
        disp(['Interception occurs for rate of change of alpha_T = ', num2str(pi/6), ' at time t = ', num2str(te), ' seconds']);
        ...
        [R_miss, idx] = min(R);
        tmiss = t(idx);
        disp(['No interception for rate of change of alpha_T = ', num2str(pi/6), '. Miss distance = ', num2str(R_miss), ' meters at t = ', num2str(tmiss), ' seconds']);
    end
    ...
    sgtitle(['Combined Plots for rate of change of alpha_T = ', num2str(pi/6)]);
...
function dydt = PP(t, y, VP, VT)
    ...
    alpha_T = y(5); % Maneuvering target
    dalpha_T_dt = pi/6;
    % \pi/6=0 for Constant target heading
    ...
    dydt = [dR_dt; dtheta_dt; dV_R_dt; dV_theta_dt; dalpha_T_dt];
end
