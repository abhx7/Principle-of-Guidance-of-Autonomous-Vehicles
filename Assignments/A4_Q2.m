...
T_end = 200; % End time for simulation (adjust if needed)
...
b = 0; % Maneuvering parameter

%[t, state, te] = ode45(@(t, y) TPN(t, y, c, b, 0, 0), tspan, initial_conditions, options);
RTPN = 1;
Nd = 3;
[t, state, te] = ode45(@(t, y) TPN(t, y, 0, b, Nd, 1), tspan, initial_conditions, options);
...
% Calculate lateral acceleration a_T
a_T = b ./ max(V_theta, e);
...
function dydt = TPN(t, y, c, b, Nd, RTPN)
    ...
    dV_R_dt = -dtheta_dt * V_theta - a_P*sin(alpha_P-theta)*cos(alpha_P-theta);              % Radial velocity rate
    dV_theta_dt = dtheta_dt * V_R + a_T - a_P*power(sin(alpha_P-theta),2); % Angular velocity rate
    dalpha_P_dt = a_P * cos(alpha_P - theta)/V_P;   % Pursuer heading angle rate
